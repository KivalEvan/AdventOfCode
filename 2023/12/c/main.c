...
int hashmap_has(const Hashmap *hashmap, const char *str) {
   int idx = hash(str) % BUCKET_SIZE;
   Node *n = hashmap->bucket[idx];
   while (true) {
      if (n == NULL)
...
long long memodrecursion(Hashmap *cache, const char *restrict field,
                         const int *conditions, const int c_sz, const int f_sz,
                         int f_i, int c_i);
...
long long wedoabitofrecursion(Hashmap *cache, const char *restrict field,
   ...
   if (f_i >= f_sz)
      return c_i == c_sz ? 1 : 0;
   if (field[f_i] == '.')
      return memodrecursion(cache, field, conditions, c_sz, f_sz, f_i + 1, c_i);
   if (field[f_i] == '#')
...
long long lookahead(Hashmap *cache, const char *restrict field,
   ...
   if (f_sz - 1 == conditions[c_i])
      return memodrecursion(cache, field, conditions, c_sz, f_sz, f_sz,
                            c_i + 1);
...
long long memodrecursion(Hashmap *cache, const char *restrict field,
   ...
   if (n != NULL) {
      ...
      long long result =
          wedoabitofrecursion(cache, field, conditions, c_sz, f_sz, f_i, c_i);
