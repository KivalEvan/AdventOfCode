...
def get_tuple(xdd: str) -> [int, str]:
   x = xdd.strip().split(' ')
   return [int(x[0]), x[1]]

def get_sequences(game: str) -> List[[int, str]]:
   return [get_tuple(x) for x in game[game.index(':') + 1:].replace(';', ',').split(',')]

def do_part_one(cubes: List[[int, str]], idx: int) -> int:
   rgb = [12, 13, 14]
   for cube in cubes:
      i = ord(cube[1][0]) % 3
      if cube[0] > rgb[i]:
...
def do_part_two(cubes: List[[int, str]]) -> int:
   rgb = [0, 0, 0]
   for cube in cubes:
      i = ord(cube[1][0]) % 3
      if cube[0] > rgb[i]:
