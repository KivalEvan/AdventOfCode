...
fn parseInput(input: []const u8) [][]i64 {
    ...
    var ary = std.ArrayList([]i64).init(std.heap.page_allocator);
    while (lines.next()) |line| {
        var numstr = std.mem.splitSequence(u8, line, " ");
        var nums = std.ArrayList(i64).init(std.heap.page_allocator);
...
fn part1(input: []const u8, is_test: bool) []const u8 {
    ...
    defer std.heap.page_allocator.free(histories);
