...
def print_result(solution: SolutionWrapper) -> SolutionWrapper:
   if solution.iteration == 1:
      ...
      print(
          f"Timer: {round(solution.bench[0][2], 3)} > {round(solution.bench[1][2], 3)} > {round(solution.bench[2][2], 3)}"
      )
   else:
      print(f"\n{solution.tag}: (ms) min..max avg")
      print(
          f"IO: {round(solution.bench[0][0], 3)} .. {round(solution.bench[0][1], 3)} - {round(solution.bench[0][2], 3)}"
      )
      print(
          f"Part: {round(solution.bench[1][0], 3)} .. {round(solution.bench[1][1], 3)} - {round(solution.bench[1][2], 3)}"
      )
      print(
          f"Overall: {round(solution.bench[2][0], 3)} .. {round(solution.bench[2][1], 3)} - {round(solution.bench[2][2], 3)}"
      )
...
def execute(solution: SolutionWrapper) -> SolutionWrapper:
   ...
   input = (solution.path
            if solution.options.get("has_io") else get_input(solution.path))
...
def perform(solution: SolutionWrapper) -> SolutionWrapper:
   ...
   solution.bench = [
       [min(elapsed_io),
        max(elapsed_io),
        sum(elapsed_io) / len(elapsed_io)],
       [
           min(elapsed_part),
           max(elapsed_part),
           sum(elapsed_part) / len(elapsed_part)
       ],
       [
           min(elapsed_overall),
           max(elapsed_overall),
           sum(elapsed_overall) / len(elapsed_overall),
       ],
   ]
...
def run(
    args: List[str],
    part1: Callable[[str, bool], str],
    part2: Callable[[str, bool], str],
    options: Dict,
):
   path = args[1]
   path_answers = os.path.join(path, "answers.txt")
   path_input_test1 = os.path.join(path, "test1.txt")
   path_input_test2 = os.path.join(
       path, ("test2.txt" if options.get("has_alternate") else "test1.txt"))
   ...
   solutions = [
       SolutionWrapper(
           tag="Test 1",
           func=part1,
           path=path_input_test1,
           test=answers.get("test1"),
           iteration=iteration,
           options=options,
       ),
       SolutionWrapper(
           tag="Part 1",
           func=part1,
           path=path_input_main,
           test=answers.get("part1"),
           iteration=iteration,
           options=options,
       ),
       SolutionWrapper(
           tag="Test 2",
           func=part2,
           path=path_input_test2,
           test=answers.get("test2"),
           iteration=iteration,
           options=options,
       ),
       SolutionWrapper(
